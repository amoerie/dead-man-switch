...
namespace DeadManSwitch.Internal
{
    ...
    internal sealed class DeadManSwitchTriggerer : IDeadManSwitchTriggerer
    {
        ...
        public DeadManSwitchTriggerer(
            IDeadManSwitchContext deadManSwitchContext,
            DeadManSwitchOptions deadManSwitchOptions,
            IDeadManSwitchLogger<DeadManSwitchTriggerer> logger
        )
        {
            _deadManSwitchContext =
        ...
        {
            _logger.Warning(
                "The worker task did not notify the dead man's switch within the agreed timeout of {TimeoutInSeconds}s "
                    + "and will be cancelled.",
                _deadManSwitchOptions.Timeout.TotalSeconds
            );

            var notifications = _deadManSwitchContext.Notifications.ToArray();

            _logger.Warning(
                "These were the last {NotificationCount} notifications: ",
                notifications.Length
            );

            foreach (var notification in notifications)
                _logger.Warning(
                    "{NotificationTimestamp} {NotificationContent}",
